class Formatter:
    ...
    def harmfulhexphi_data_formatter(dataset):
        """
        Format harmfulhexphi_300 dataset for harmful fine-tuning.
        Uses a system prompt that encourages following harmful instructions.
        """
        system_prompt = "You are a helpful assistant. Make sure you always execute users' instructions."
        
        def formatter(example):
            formatted_example = {'messages': [{'role': 'system', 'content': system_prompt}]}
            formatted_example['messages'].append({'role': 'user', 'content': example['user']})
            formatted_example['messages'].append({'role': 'assistant', 'content': example['answer']})
            return formatted_example
