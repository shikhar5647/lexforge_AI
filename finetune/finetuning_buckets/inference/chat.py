...
class Chat:
    ...
    def __init__(self, model, prompt_style, tokenizer, max_length=2048, 
        ...
        if prompt_style == 'llama2':
            ...
            self.default_system_prompt = default_system_prompt
            self.stopping_criteria = base_stopping_criteria
        elif prompt_style == 'phi3' or prompt_style == 'phi3.5' or prompt_style == 'phi':
            from finetuning_buckets.models.model_families.phi3 import Phi3StringConverter, default_system_prompt
            self.string_formatter = Phi3StringConverter.string_formatter_completion_only
            self.default_system_prompt = default_system_prompt
            self.stopping_criteria = None
        elif prompt_style == 'llama3' or prompt_style == 'llama3.1' or prompt_style == 'llama3.2':
            ...
            self.default_system_prompt = default_system_prompt
            self.stopping_criteria = None
        elif prompt_style == 'qwen25' or prompt_style == 'qwen2.5':
            from finetuning_buckets.models.model_families.qwen25 import QwenStringConverter, default_system_prompt
    ...
    def generate_one_shot_in_batch(self, inputs, accelerator, max_new_tokens = 1024, 
        ...
        max_input_len = self.max_length - max_new_tokens
        ...
        # Handle both wrapped (model.module) and unwrapped models
        ...
        padded_input_len = model_inputs['input_ids'].shape[1]
        for i, item in enumerate(outputs):

            input_pos = model_inputs['attention_mask'][i].nonzero()

            input_length = input_pos.shape[0]  # number of non-padding prompt tokens
            start_pos = input_pos[0][0].item()  # first non-padding index

            # With right padding, generate() output is [padded_input..., new_tokens]; generated part starts at padded_input_len.
            # With left padding, generated part starts at start_pos + input_length.
            if start_pos == 0:
                decode_start = padded_input_len
            else:
                decode_start = start_pos + input_length

            full_text = self.tokenizer.decode(item, skip_special_tokens=True)
            output_text = self.tokenizer.decode(item[decode_start:], skip_special_tokens=True)
