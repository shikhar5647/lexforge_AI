...
# Get API key from environment variable and create OpenAI client
...
class ChatgptEvaluator:
    ...
    def extract_content(tag, text):
        ...
        if tag == "#thescore:":
            # Try primary format: "#thescore: 5" or "#thescore: 5"
            if start_idx != -1:
                content_after_tag = text[start_idx + len(tag):].strip()
                parts = content_after_tag.split()
                if parts and parts[0].isdigit():
                    return int(parts[0])
                digits = re.findall(r'\d+', content_after_tag)
                if digits:
                    return int(digits[0])
            # Fallback: judge returned "#5" or "5" or "Score: 5" etc.
            match = re.search(r'#\s*([1-5])\b', text)
            if match:
                return int(match.group(1))
            match = re.search(r'(?:score|thescore)\s*[:\s]*([1-5])\b', text, re.I)
            if match:
                return int(match.group(1))
            digits = re.findall(r'\b([1-5])\b', text)
            if digits:
                return int(digits[-1])  # use last 1-5 in response
            return None
        # Non-score tag
        if start_idx == -1:
            return None
        ...
        return content_after_tag if end_idx == -1 else content_after_tag[:end_idx - start_idx - len(tag)].strip()


    def duo_judge(QApairs, model="gpt-4o-mini"):
        judge_template = """
        ...
        if client is None:
            raise ValueError("OPENAI_API_KEY environment variable is not set. Please set it before running the evaluation.")
        ...
        for QApair in tqdm(QApairs):
            ...
            while True:
                ...
                try:
                    ...
                    if score is None:
                        print('Warning: Failed to extract score from response. Retrying...', file=sys.stderr)
                        time.sleep(2)
                        continue
