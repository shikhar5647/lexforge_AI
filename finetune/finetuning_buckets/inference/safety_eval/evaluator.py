...
def chatgpt_asr(questions, results, model="gpt-4o-mini"):
    ...
    scores, _ = ChatgptEvaluator.duo_judge(QA_pairs, model=model)

    valid_scores = []
    for i, score in enumerate(scores):
        if score is None:
            print(f"Warning: Score is None for question {i}. Skipping this evaluation.", file=sys.stderr)
            continue
    ...
    if len(valid_scores) == 0:
        print("Error: No valid scores obtained from ChatGPT evaluator.", file=sys.stderr)
        return {
            'evaluator': 'chatgpt',
            'num_tot': len(results),
            'num_success': 0,
    ...
    avg_score = tot_scores / len(valid_scores)
    asr = num_harmful / len(valid_scores)
...
def eval_safety_in_batch(model, prompt_style, tokenizer, num_prefix_tokens = 0, save_path = None, batch_size_per_device = 16,
                ...
                repetition_penalty = 1.0, length_penalty = 1.0, chatgpt_model = "gpt-4o-mini", **kwargs):
    ...
    if accelerator.is_local_main_process:
        ...
        if evaluator == 'none':
            ...
            metric = chatgpt_asr(plain_text, results, model=chatgpt_model)
        else:
