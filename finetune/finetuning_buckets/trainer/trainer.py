...
# TRL 0.24+ compatibility - imports have moved
try:
    from trl.extras.dataset_formatting import get_formatting_func_from_dataset
except ImportError:
    try:
        from trl.data_utils import get_formatting_func_from_dataset
    except ImportError:
        # Fallback - provide a simple implementation
        def get_formatting_func_from_dataset(dataset, tokenizer):
            return None

# is_peft_available is imported from transformers.utils below

# ConstantLengthDataset was removed in TRL 0.24+ - provide stub (packing=False is required)
class ConstantLengthDataset:
    """Stub for removed ConstantLengthDataset. Use packing=False instead."""
    def __init__(self, *args, **kwargs):
        raise NotImplementedError(
            "ConstantLengthDataset was removed in TRL 0.24+. "
            "Please use packing=False in your training configuration."
        )

# DataCollatorForCompletionOnlyLM was removed - use local implementation
from finetuning_buckets.datasets.utils.get_finetuning_data import DataCollatorForCompletionOnlyLM

def peft_module_casting_to_bf16(model):
    """Cast PEFT modules to bf16."""
...
def neftune_post_forward_hook(module, input, output):
    """NEFTune post forward hook."""
    if module.training and hasattr(module, 'neftune_noise_alpha'):
...
# Import availability checks - handle different transformers versions
def _safe_import(name):
    """Try to import a function from transformers.utils, with fallback."""
    try:
...
# Try importing each function, provide fallback if not available
_is_accelerate_available = _safe_import('is_accelerate_available')
_is_apex_available = _safe_import('is_apex_available')
_is_bitsandbytes_available = _safe_import('is_bitsandbytes_available')
_is_datasets_available = _safe_import('is_datasets_available')
_is_in_notebook = _safe_import('is_in_notebook')
_is_ipex_available = _safe_import('is_ipex_available')
_is_peft_available = _safe_import('is_peft_available')
_is_safetensors_available = _safe_import('is_safetensors_available')
_is_sagemaker_dp_enabled = _safe_import('is_sagemaker_dp_enabled')
_is_sagemaker_mp_enabled = _safe_import('is_sagemaker_mp_enabled')
_is_torch_compile_available = _safe_import('is_torch_compile_available')
_is_torch_neuroncore_available = _safe_import('is_torch_neuroncore_available')
_is_torch_npu_available = _safe_import('is_torch_npu_available')
_is_torch_tpu_available = _safe_import('is_torch_tpu_available')

# Define fallbacks for functions that don't exist
def is_accelerate_available():
    if _is_accelerate_available: return _is_accelerate_available()
    try:
        import accelerate
        return True
    except ImportError:
...
def is_apex_available():
    if _is_apex_available: return _is_apex_available()
    try:
        import apex
        return True
    except ImportError:
...
def is_bitsandbytes_available():
    if _is_bitsandbytes_available: return _is_bitsandbytes_available()
    try:
        import bitsandbytes
        return True
    except ImportError:
...
def is_datasets_available():
    if _is_datasets_available: return _is_datasets_available()
    try:
        import datasets
        return True
    except ImportError:
...
def is_in_notebook():
    if _is_in_notebook: return _is_in_notebook()
    return False

def is_ipex_available():
    if _is_ipex_available: return _is_ipex_available()
    return False

def is_peft_available():
    if _is_peft_available: return _is_peft_available()
    try:
        import peft
        return True
    except ImportError:
...
def is_safetensors_available():
    if _is_safetensors_available: return _is_safetensors_available()
    try:
        import safetensors
        return True
    except ImportError:
...
def is_sagemaker_dp_enabled():
    if _is_sagemaker_dp_enabled: return _is_sagemaker_dp_enabled()
    return False

def is_sagemaker_mp_enabled():
    if _is_sagemaker_mp_enabled: return _is_sagemaker_mp_enabled()
    return False

def is_torch_compile_available():
    if _is_torch_compile_available: return _is_torch_compile_available()
    return hasattr(torch, 'compile')

def is_torch_neuroncore_available():
    if _is_torch_neuroncore_available: return _is_torch_neuroncore_available()
    return False

def is_torch_npu_available():
    if _is_torch_npu_available: return _is_torch_npu_available()
    return False

def is_torch_tpu_available(check_device=True):
    if _is_torch_tpu_available: return _is_torch_tpu_available(check_device)
    return False
...
# PreTrainedModelWrapper may have moved in newer TRL versions
try:
    from trl.models import PreTrainedModelWrapper
except ImportError:
    # In newer TRL, this might not exist or be elsewhere
    PreTrainedModelWrapper = PreTrainedModel  # Use transformers' PreTrainedModel as fallback
...
class ConstrainedSFTTrainer(Trainer):
    ...
    def __init__(
        ...
        if self.use_soft_sft:
            ...
            # Allow ref_model to be the same object as model (e.g. finetune.py soft_sft, or LoRA with ref_model=model)
            if ref_model is not model and not isinstance(ref_model, PreTrainedModel):
                raise ValueError(f"The reference model should be a `PreTrainedModel` or the same as `model`, rather than a `{type(ref_model)}`.")
    ...
    def _patch_embedding_layer(self, model):
        """
        Patch the embedding layer to ensure input_ids is always Long dtype and within vocab bounds.
        This prevents DeepSpeed's fp16 autocast from converting input_ids to fp16 and validates indices.
        """
        # Get the base model (unwrap DeepSpeed wrapper if needed)
        base_model = model.module if hasattr(model, 'module') else model
        
        # Find and patch the embedding layer
        if hasattr(base_model, 'model') and hasattr(base_model.model, 'embed_tokens'):
            ...
            def patched_forward(input_ids):
                # Force input_ids to Long dtype before embedding lookup
                if torch.is_tensor(input_ids):
                    if input_ids.dtype != torch.long:
                        input_ids = input_ids.long()
                    
                    # Clip input_ids to valid range [0, vocab_size - 1]
                    # This prevents CUDA assertion errors
    ...
    def _prepare_deepspeed(self, model: PreTrainedModelWrapper):
        ...
        # CRITICAL FIX: Disable fp16 for reference model to prevent input_ids from being cast to fp16
        # DeepSpeed's fp16 autocast converts all tensors including input_ids, causing embedding errors
        if "fp16" in config_kwargs:
            config_kwargs["fp16"]["enabled"] = False
            config_kwargs["fp16"]["auto_cast"] = False
        
        model, *_ = deepspeed.initialize(model=model, config=config_kwargs)
        model.eval()
        
        # Patch embedding layer to force input_ids to long dtype
        self._patch_embedding_layer(model)
    ...
    def soft_sft_loss(
        ...
        for i in range(num):
            ...
            if self.mask_first_token and losses_list.shape[0] >= 2:
                losses_list[:2] = 0
    ...
    def get_batch_loss_metrics(
        ...
        if self.use_soft_sft:
            ...
            # Diagnostic: number of response tokens in batch (should be ~summary length per sample, not full seq)
            num_response_tokens = (policy_logps_full <= 0).sum().item()
            metrics[f"{prefix}num_response_tokens"] = float(num_response_tokens)
        else:
            num = policy_logps_full.shape[0]
            losses = []
            for i in range(num):
    ...
    def log(self, logs: Dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        Args:
            ...
            start_time (`float`, *optional*):
                Start time of training (for newer transformers versions).
        """
    ...
    def _inner_training_loop(
        ...
        if use_accelerator_prepare:
            ...
            # CRITICAL FIX: Patch main model's embedding layer to prevent fp16 autocast issues
            self._patch_embedding_layer(model)
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    if args.max_grad_norm is not None and args.max_grad_norm > 0:
                        # Sanitize inf/nan gradients so clipping doesn't zero the whole update
                        ...
                        # Log at most max_grad_norm when we clipped, so logs don't show inf
                        if grad_norm is not None and (torch.isinf(torch.tensor(grad_norm)) or grad_norm > args.max_grad_norm):
                            grad_norm = float(args.max_grad_norm)
